...
{
    public class CheckEnemyInShortAttackRange_State : Condition
    {
        ...
        public CheckEnemyInShortAttackRange_State(Transform transform)
        {
        ...
        protected override NodeState OnEvaluate(Transform agent, Blackboard blackboard)
        {
            ...
            if (EnemyAI.isAttacking || Vector3.Distance(_transform.position, target.position) <= blackboard.Get<float>("shortAttackRange"))
            {
